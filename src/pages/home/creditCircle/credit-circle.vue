<template>
	<div id="credit" class="wrap bg-f0 h100p" > 
        <i-header :title="title" ></i-header>
         <!-- <x-header :left-options="{showBack: false}">交流圈</x-header> -->
		<div class="fixed-content" ref="main" style="height:100%;overflow: scroll;">        
			<mt-loadmore :top-method="loadTop" :distanceIndex="5"  :autoFill="true" topPullText="↓ 下拉刷新" topDropText="↑ 释放更新" topLoadingText="正在加载 ..." :topDistance="70" ref="loadmore">
					<div class="mui-table-view" >
                        <section class="li bg-f0 clearfix" v-for="(item,index) in list" :key="index" v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" :infinite-scroll-distance="50">
                            <div class="right-con ">
                                <i class="lazy-c bg-f5 img-wrap-left"><img  alt=""  :src="item.head_img?(imgUrl+item.head_img):''" :onerror='defaultImg'></i>
                                    ...
                                    <li class="content-com" v-for='(i,n) in comlist' v-if="comlistshow&&item.id==i.post_id"  :key="n">
                                        <div class="content-inner" @click="addcommentinside(item.id,n,i.id,i.nickname,i.is_my_comment,index)">
                                            <a href="javascript:void(0)">{{i.nickname}}</a>
                                             <a href="javascript:void(0);" v-if='!i.nickname'>匿名用户</a>
                        ...
                        <!-- <div class="input-all"  :class="activeinputall?'':'write-phonenum-1000'" ref='inputall'>
                                    <textarea  class="activeinput "  ref="inputfocusfff"  v-model="textmodel"  id="textarea"  @blur="offFocus" >
                                    </textarea>
...
export default {
    ...
    components: {
        iHeader,LoadMore,Popup,Lazyload,Actionsheet,ConfirmPlugin,Confirm
    }, 
    created() {
        this.getList();
        //点赞状态
                    ...
                    fn: (code, msg) => {
                        switch (code) {
                            case 200:
                                ...
                                if(msg.list.length<=msg.pageSize){
                                    this.more_state = 2;
                                    this.loading = true;
                                }
                                this.list = [...this.list,...msg.list];
                                break;
                            case -1:
                                 this.reloadState = 2;
                                 this.$router.push('/my/login');
    ...
    //    comment(){
    //             const _this=this;
    //             var content = `<div><textarea v-model="textmodel"></textarea></div>`;
    //             // console.log(content)
    //             this.$vux.confirm.show({
    //                 content:content,
    //                 // value:_this.textmodel  ,
    //                 confirmText:'评论',
    //                 cancelText:'取消', 
    //                 // onShow(){
    //                 //     _this.$vux.confirm.setInputValue("请输入评论内容") 
    //                 // },              
    //                 onCancel () {                  
    //                 },
    //                 onConfirm () {
...
#credit .img-wrap-left{width:50px;height:50px;position: absolute;left:10px;top:10px;}
#credit .img-wrap-left img{width:100%;height:100%;}
#credit .right-con{width: 100%;padding-left:70px;padding-top:12px;position:relative;}
.right-content{position:relative;width:70%;}
.right-comment{position:absolute;right:0;z-index:30;}
...
<style>
.weui-dialog__ft {position: relative;line-height: 35px;font-size: 18px;}
.vux-prompt {
    padding-bottom: 0;
    line-height: 65px;
}
/* .vux-prompt input{height: 100%;border: none;border-bottom: 1px solid #3333;} */
#credit .weui-dialog__bd:first-child{padding:8px;}
#credit .input-all{position: fixed;width:100%;z-index:10000;background:#f0f0f0;bottom:0;padding:5px 2%;min-height:40px;}
#credit .input-all .activeinput{max-height:150px;margin-left: 0;text-align: left;height:30px;width:80%;display: block;float:left;border:none;border-bottom: 1px solid #469dc8;background: #f0f0f0;border-radius:0;margin-bottom:0;padding:0;padding-bottom:3px;}
#credit .input-all input{height: 30px;width: 15%;background: #469dc8;border: 1px solid #469dc8;float: right;text-align: center;margin-left: 1%;position: absolute;right: 2%;top: 50%;transform: translateY(-50%);}
#credit .mint-popup{height:70px;line-height: 70px;background: rgba(0,0,0,.6);width: 100%;text-align: center;opacity: 1;color: #fff;}
#credit .mint-popup-top{z-index: 999999999 !important;}
#credit .v-modal{background:transparent;z-index: 0;height:70px;}
#credit .mint-tab-container{    padding-top: 43px;height:100%;}
#credit .mint-loadmore{height: 100%;overflow-y: auto;}
#credit .mint-loadmore-top,#credit .mint-loadmore-text{font-size: 14px;}
#credit .mint-tab-container-wrap,.mui-content{height: 100%;}
#credit .page-infinite-loading{text-align: center;margin-bottom: 0;}
#credit .page-infinite-loading span{display: inline-block;vertical-align: middle;margin-right: 5px;}
.write-phonenum-1000{bottom:-1000px !important;}
.mui-preview-header .mui-preview-indicator{line-height:1;width:100%;}
#credit  .input-all input[readonly=readonly] {background: #aaa;border: 1px solid #aaa;}
.mui-preview-header {padding-top: 35px;}
