<template>
	<div id="payresult">
        <x-header title="支付结果" class="fixed-top" :left-options="{backText: ''}"></x-header>
	...
	export default {
        ...
        methods:{
			 alertSet() {
			    ...
			    document.getElementById("js-alert-head").innerHTML ='即将显示支付结果';
			    var t = 10,
				...
				this.$axios.post('api/Cardpay/getCardpayInfo',{order_no:this.order_no})
					.then(res=>{
						setTimeout(function () {
							if(res.code==200){
								document.getElementById("js-alert-head").innerHTML =""
								document.getElementById("js-alert-p").innerHTML =res.msg;
							}else{
								mui.toast(res.msg)
								return false;
							}
