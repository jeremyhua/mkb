        ...
        Upload.prototype = {
            ...
            startUpload: function() {
               ...
               // var formData = new FormData();
                ...
                //});
                setTimeout(function(){
                    if(_this.fileBase.length > 0){
                        //console.log(_this.fileBase.length) 
                        var xhr = new XMLHttpRequest();
                        var databy='';
                        xhr.open(_this.config.type, _this.config.path + datas, true);
                        xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
                        xhr.send("filelist="+JSON.stringify(_this.fileBase));
                ...
                //console.log("正在上传的文件：", formData, JSON.parse(this.fileList))
                                ...
                                if (!_this.config.isRepeat) {
                                    ...
                                    if (!isRepeat) {
                                        ...
                                        reader.onload = function () {
                                            _this.fileBase.push(this.result);
                                        };
                                        reader.readAsDataURL(_self[i]);
                                    }
                                } else {
                                    _this.fileList.push(_self[i]);
                                    _this.fileSrc.push(src);
